...
- 自即日起，所有新增功能預設需納入手機相容性評估（iOS / Android），不可只以桌面體驗作為完成條件。
- 任何涉及 UI / 互動 / 渲染 / 資產的變更，需執行 mobile checklist（事件、響應式、互動替代、效能、多模態）並回寫結果。
- 重大功能開發採三段式流程：
...
- 若本版功能存在手機端暫不支援項目，必須同步在 `RoadMap.md` 與 `dev_log.md` 註明原因與預計補齊版本。
