...
- 語法設計規格（待實作）：
  - 新增 `fc` 包裹語法，支援在連續段落中明確框選閃卡範圍
  - 單行卡規格：`{{fc:: 正面 :: 背面 /fc}}`
  - 多行/清單卡規格：起始 `{{fc::`，內容沿用 `正面 ::` + 縮排答案，結尾 `/fc}}`
  - 單行與多行統一使用 `/fc}}` 作為結尾，降低記憶成本與規則分裂
  - 解析保護規則：`code block` / `inline code` / `math` 區塊內不解析 `fc` 包裹語法
  - 相容策略：第一階段保留舊語法，待新語法穩定後再評估是否進一步收斂
- 語法實作路線（建議）：
  - P0：在 parser 增加 `fc` 包裹範圍切分，不改既有舊語法行為
  - P1：補上單行與多行包裹語法的 parser 測試（含錯誤邊界）
  - P1.5：新增編輯器命令 `Wrap selection as flashcard`，支援將選取內容一鍵包裝為 `{{fc:: ... /fc}}`（並可綁定快捷鍵）
  - P2：UI 診斷與錯誤提示（未閉合 `/fc}}`、區塊內語法衝突）
  - P3：視實測結果評估是否提供語法遷移工具
- V0.2 開發順序優先級建議：
