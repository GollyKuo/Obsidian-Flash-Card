...
- Step 7：衝突副本防呆（規劃中）
  - `pre-commit` 新增檢查：若存在 `*[conflicted*]` 檔案則阻擋提交
  - 將衝突檔視為同步/流程異常訊號，需先清理再進入發版流程
...
- `Cards` 分區的功能與資料模型細節，統一由「V0.3：卡片庫管理系統主線」維護
- 與資料層保持分責：UI 可整合，但管理查詢與分析查詢維持模組邊界
- 規劃已納入：2026-03-29，V0.1.5（先加入 RoadMap，尚未執行）

### Sprint E.5: 大規模卡片量性能與延遲控制（高優先，規劃中）

目標：在大量筆記與大量卡片情境下，維持 `Cards` 分區、同步與複習流程的可用性與低延遲，避免 V0.3 之後再進行高成本回補。

- 本節性能細項已整併至「V0.3-P3 / V0.3-P4」，避免多處重複維護
- 本節保留定位：提醒在 V0.3 完成前，性能補強不可後置
...
## V0.2：手機相容架構（Mobile Compatibility）

目標：確保插件在 Obsidian 手機版（iOS / Android）可用、可讀、可維護，不因桌面假設導致互動失效或效能退化。

### V0.2 範圍

- 事件輸入層相容：以 `pointer` 事件為主，補齊 touch 情境 fallback，避免只綁 `mousedown`。
- 平台能力層：建立 `Mobile/Desktop Capability Layer`，集中管理 hover / keyboard / pointer 等能力判斷，避免分散在各模組。
- 手機 UI 響應式：Review Modal / 設定頁在小螢幕可操作（按鈕尺寸、間距、排版層級）。
- 互動模式差異：移除對 hover 的依賴，鍵盤快捷鍵提供等價替代操作。
- 行動效能與耗電：控制高亮重算與同步頻率，避免長文檔/大量卡片造成卡頓與耗電。
- 多模態資產策略：圖片/音訊在手機端有壓縮、快取、清理策略（容量與傳輸成本可控）。
- 測試矩陣：建立 iOS / Android 核心手測清單，納入發版前必測。

### V0.2 分階段優先級

- V0.2-P0：建立 mobile capability checklist（事件、UI、效能、資產、測試）與 `Mobile/Desktop Capability Layer`。
- V0.2-P1：輸入事件與互動相容層（pointer/touch + reveal/edit 行為一致性），先覆蓋 `AnswerHighlighter` 與 `BlockIdManager`。
- V0.2-P2：Review Modal 與設定頁 mobile 響應式調整。
- V0.2-P3：行動裝置效能治理（高亮/同步節流、重算控制、耗電觀察）。
- V0.2-P4：多模態資產 mobile 策略（壓縮、清理、同步行為）。
- V0.2-P5：iOS / Android 測試矩陣與發版驗收流程。

### V0.2 重大先行優化（開工前先做）

- 必做 1：手機事件層抽象（`pointer` 為主、touch fallback），避免既有 `mousedown/click` 桌面假設。
- 必做 2：平台能力層（Capability Layer）先落地，避免 mobile 判斷邏輯分散造成後續重工。
- 必做 3：Review Modal / 設定頁先完成最小可用 mobile 響應式骨架，再疊加新功能。
- 必做 4：提交流程加入衝突檔防呆（`*[conflicted*]`）以避免版本流程污染。

### V0.2 驗收重點

- 手機端可完整完成「編輯 -> 產卡 -> 複習」主流程。
- 點擊/觸控互動一致且不出現誤觸切換。
- 大筆記與多卡片情境下，操作延遲與耗電在可接受範圍。
- 多模態資產在手機端可正常顯示/播放，且不造成容量失控。

---

## V0.3：卡片庫管理系統主線（整合規劃）

目標：先完成可日常使用的卡片庫管理能力，再進入 V0.5 的 AI 強化，避免流程倒掛。
本章為 V0.3 卡片庫管理系統的唯一主規劃來源；Sprint 區塊僅保留摘要與里程碑定位。

### V0.3 範圍（整併自 Sprint E / Sprint E.5）
...
- V0.3-P0：`Cards Query Layer` + 來源健康度模型 + `inheritedTags / cardTags / effectiveTags` 查詢出口
- V0.3-P1：`Cards` 分區 UI 第一版（分組檢視預設、列表檢視切換、搜尋、篩選、排序、來源跳轉）
- V0.3-P2：批次標籤操作、群組統計、細節抽屜／卡片詳情面板
- V0.3-P3：性能補強（虛擬清單、延遲載入、group lazy expand、derived tag/source indexes）
- V0.3-P4：benchmark/profiling 基線與延遲門檻，作為後續擴充的進入條件

### V0.3 與 V0.5 邊界

- V0.3 先完成卡片管理核心與性能底座
- V0.5 再承接 AI enrich、批次 AI、多模態資產

### V0.3 與 V0.4 邊界

- V0.3 聚焦「卡片庫管理」：查詢、分組、標籤、來源健康度、資料與效能模型
...
目標：專注於「複習時」閃卡本身的視覺與互動樣式，不與 V0.3 卡片庫管理功能耦合。
...
本章為 V0.5 AI 規劃主來源；與 AI 相關的 Sprint 僅作歷史對照（Sprint H/I）。

### V0.5 前置條件（已完成，來自 V0.1.x）
...
### V0.5 互動模式規劃（Auto + Manual + Hybrid）
...
### V0.5-P1（原 Sprint H）：單卡 AI 豐富化（先做）
...
6. V0.2: 手機相容架構（Mobile Compatibility）
7. V0.3: 卡片庫管理系統主線（整合規劃）
8. Sprint F: 學習體驗強化
9. Sprint G: 行尾 HUD / 狀態提示
10. V0.4: 複習閃卡呈現樣式主線（整合規劃）
11. V0.5-P1: 單卡 AI 豐富化（低風險高價值）
12. V0.5-P2: 學習分析與回顧增強
13. V0.5-P3: 批次 AI 與多模態
14. v2.0 之後的長期功能
